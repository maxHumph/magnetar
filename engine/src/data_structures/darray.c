...
void _darray_set_field(void* darray, u64 field, u64 val) {
  u64* head = (u64*)darray - DARRAY_FIELD_LENGTH;
  head[field] = val;
}

void* _darray_resize(void* darray) {
  u64 length = _darray_get_field(darray, DARRAY_LENGTH);
  u64 stride = _darray_get_field(darray, DARRAY_STRIDE);
  void* temp = _darray_create(DARRAY_RESIZE_FACTOR *
                              _darray_get_field(darray, DARRAY_CAPACITY),
                              stride);
...
void* _darray_push(void* darray, const void* val_ptr) {
  u64 length = _darray_get_field(darray, DARRAY_LENGTH);
  u64 stride = _darray_get_field(darray, DARRAY_STRIDE);
  if (length >= _darray_get_field(darray, DARRAY_CAPACITY)) {
